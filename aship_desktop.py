...
class AutoHealApp(ctk.CTk):
    def __init__(self):
        ...
        self.topology_canvas = ctk.CTkCanvas(
            topology_frame, bg=CARD_BG, highlightthickness=0
        )
        ...
        for i, j in edges:
            ...
            if dist > 0:
                ...
                self.topology_canvas.create_oval(
                    px - 6, py - 6, px + 6, py + 6, fill="", outline="#00f2fe", width=1
                )
                self.topology_canvas.create_oval(
                    px - 3, py - 3, px + 3, py + 3, fill="#00f2fe", outline=""
                )
                ...
                self.topology_canvas.create_oval(
                    px2 - 3, py2 - 3, px2 + 3, py2 + 3, fill="#fe0979", outline=""
                )
        r_w, r_h = 75, 28
    ...
    def build_terminal_frame(self):
        ...
        ctk.CTkLabel(
            frame,
            text="Live Terminal (Real-time AI Tracing)",
            font=ctk.CTkFont(family="Inter", size=20, weight="bold"),
            text_color=TEXT_MAIN,
