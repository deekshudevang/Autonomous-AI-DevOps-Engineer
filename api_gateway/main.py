...
from agent_core.main import extract_metrics_ast, reason_and_diagnose, validate_policy, execute_sandboxed_tool, AgentState
...
async def run_agent_workflow(incident_id: str, payload: dict):
    ...
    IN_MEMORY_DB[incident_id].trace.append(f"> [PROMETHEUS] Ingested raw payload: {payload['alerts'][0]['labels']}")
    IN_MEMORY_DB[incident_id].status = "analyzing"
    await asyncio.sleep(1)
    
    # Node 1: Extract AST
    state = extract_metrics_ast(state)
    IN_MEMORY_DB[incident_id].trace.append(f"> [Agent Core] Parsed AST Metrics: {state['parsed_ast_metrics']}")
    await asyncio.sleep(1)
    ...
    IN_MEMORY_DB[incident_id].trace.append(f"> [Analytics] Spectrum-Based Fault Localization executed.")
    IN_MEMORY_DB[incident_id].trace.append(f"> [Analytics] BAYESIAN POSTERIOR CONFIDENCE: {conf:.4f}")
    IN_MEMORY_DB[incident_id].trace.append(f"> [Agent Core] Proposed Remediation: {state['proposed_action']}")
    ...
    IN_MEMORY_DB[incident_id].trace.append(f"> [Policy Engine] Decision: {'APPROVED' if state['policy_approved'] else 'REJECTED'}")
    IN_MEMORY_DB[incident_id].status = "sandboxing" if state["policy_approved"] else "halted"
    await asyncio.sleep(1)
    ...
    IN_MEMORY_DB[incident_id].trace.append(f"> [Sandbox Execution] Result: {state['final_result']}")
