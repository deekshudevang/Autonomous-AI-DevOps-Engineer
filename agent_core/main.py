...
class FaultAnalyzer:
    ...
    def calculate_ochiai(failed_passed_ratio: float, total_failed: int) -> float:
        ...
        try:
            return total_failed / math.sqrt(total_failed * (total_failed + failed_passed_ratio))
        except ZeroDivisionError:
            return 0.0

    @staticmethod
    def get_bayesian_confidence(prior: float, true_positive_rate: float, false_positive_rate: float) -> float:
...
class RemediationAction(BaseModel):
    tool_name: str = Field(..., description="Deterministic tool name mapped to the highest SBFL score")
    arguments: Dict[str, Any] = Field(..., description="Extracted parameters via AST regex")
    bayesian_confidence: float = Field(..., description="Mathematical confidence score (Requires > 0.90 for OPA)")
...
def get_qdrant_client():
    ...
    try:
        ...
        logger.warning(f"[Vector DB] Qdrant unavailable: {e}. Skipping RAG historical verification.")
        return None
...
def extract_metrics_ast(state: AgentState) -> AgentState:
    ...
    state["parsed_ast_metrics"] = {"job": job, "severity": "critical", "error_code": error_code}
    state["sbfl_scores"] = {}
    return state
...
def reason_and_diagnose(state: AgentState) -> AgentState:
    ...
    ochiai_score = FaultAnalyzer.calculate_ochiai(failed_passed_ratio=0.2, total_failed=50)
    bayesian_conf = FaultAnalyzer.get_bayesian_confidence(prior=0.8, true_positive_rate=0.95, false_positive_rate=0.05)
    
    logger.info(f"[Analytics] Calculated Ochiai Score: {ochiai_score:.3f}")
    logger.info(f"[Analytics] Calculated Bayesian Posterior Confidence: {bayesian_conf:.3f}")
    
    # Qdrant Vector DB Fetch for Historical Proof
    client = get_qdrant_client()
    if client:
        hits = client.search(collection_name="incident_memory", query_vector=[0.1]*384, limit=1)
        if hits:
    ...
    proposed = RemediationAction(
        ...
        arguments={"namespace": "prod", "deployment": target_job, "detected_fault": error_code},
...
def validate_policy(state: AgentState) -> AgentState:
    logger.info(f"[Policy Engine] Verifying mathematical confidence against OPA thresholds...")
    if state["confidence"] < 0.90:
        logger.error(f"[Policy Engine] REJECTED. Bayesian Confidence {state['confidence']:.3f} < 0.90")
        state["policy_approved"] = False
    else:
        logger.info(f"[Policy Engine] APPROVED. Bayesian Confidence {state['confidence']:.3f} >= 0.90")
...
def execute_sandboxed_tool(state: AgentState) -> AgentState:
    ...
    import requests
    try:
        res = requests.post("http://localhost:5000/api/autoheal", timeout=2)
        if res.status_code == 200:
            logger.info("[AutoHeal] Successfully pushed hotfix to Demo Application.")
            state["final_result"] = f"SUCCESS: Orchestrated {action['tool_name']} on {action['arguments']}. Target application successfully patched and healed."
        else:
            state["final_result"] = f"FAILED: AutoHeal push rejected with status {res.status_code}"
    except Exception as e:
        logger.error(f"[AutoHeal] Failed to patch Demo Application: {e}")
        state["final_result"] = f"FAILED: Unable to communicate with target deployment. {e}"
